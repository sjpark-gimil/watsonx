...
async function loadAllDocs() {
  ...
  for (const lang of langs) {
    ...
    if (!fs.existsSync(base)) { byLang[lang] = []; continue; }
    const files = walk(base);
    byLang[lang] = files.map(f => {
      const raw = fs.readFileSync(f, 'utf8');
      const { data, content } = matter(raw);
      const slug = path.relative(base, f).replace(/\\/g, '/').replace(/\.(mdx?|MDX?)$/, '');
      return {
        lang,
...
function getDocBySlug(all, lang, slug) {
  return (all[lang] || []).find(d => d.slug === slug);
}

function listDocs(all, lang) {
  return (all[lang] || []).map(d => ({
    slug: d.slug,
    title: d.front.title,
